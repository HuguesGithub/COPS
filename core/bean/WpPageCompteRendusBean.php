...
class WpPageCompteRendusBean extends WpPageBean
{
  ...
  {
    if(!$this->initCompteRendu()) {
      $this->CompteRendu = new CompteRendu();
      return $this->getContentIdentification();
    }

    // On traite la soumission d'un formulaire.
    // Cette étape ne sert qu'à changer le statut du formulaire.
    // Si un seul parent est renseigné, la validation doit passer le statut du CR de STATUS_WORKING à STATUS_PUBLISHED
    // Si deux parents sont renseignés, la validation du premier parent doit passer le statut du CR de STATUS_WORKING à STATUS_PENDING
    // Si deux parents sont rensiegnés, la validation du deuxième parent doit passer le statut du CR de STATUS_PENDING à STATUS_PUBLISHED
    if (isset($_POST) && !empty($_POST) && isset($_POST[self::AJAX_SAVE])) {
      // On récupère le statut
      $statut = $this->CompteRendu->getStatus();
      $currentLogin = $_SESSION['userLogin'];
      $Auteur = $this->CompteRendu->getAuteurRedaction();
      $auteurLogin  = $Auteur->getLogin();
      $Adulte1 = $this->CompteRendu->getAdulte('first');
      if ($Adulte1->getId()==$Auteur->getId()) {
        $AutreAuteur = $this->CompteRendu->getAdulte('second');
      } else {
        $AutreAuteur = $this->CompteRendu->getAdulte('first');
      }
      $update = false;

      if ($statut==self::STATUS_WORKING && $currentLogin==$auteurLogin) {
        $update = true;
        if ($AutreAuteur->getLogin()=='') {
          $this->CompteRendu->setStatus(self::STATUS_PUBLISHED);
        } else {
          $this->CompteRendu->setStatus(self::STATUS_PENDING);
        }
      } elseif ($statut==self::STATUS_PENDING && $currentLogin==$AutreAuteur->getLogin()) {
        $update = true;
        $this->CompteRendu->setStatus(self::STATUS_PUBLISHED);
      }
      if ($update) {
