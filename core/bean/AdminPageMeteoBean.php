...
class AdminPageMeteoBean extends AdminPageBean
{
  ...
  public function buildGraph(&$strLeftTemps, &$strSectionColumns)
  {
		...
		$strSql = "SELECT MIN(temperature) as minT, MAX(temperature) as maxT FROM wp_7_cops_meteo WHERE dateMeteo LIKE '$strDateMeteo%';";
		...
		for ($i=0; $i<$nbDays; $i++) {
			...
			for ($j=0; $j<4; $j++) {
				...
				$strSql = "SELECT * FROM wp_7_cops_meteo WHERE dateMeteo = '$strDateMeteo".str_pad(1+$i, 2, '0', STR_PAD_LEFT)."' AND heureMeteo BETWEEN '".str_pad($j*6, 2, '0', STR_PAD_LEFT).":00' AND '".str_pad(($j+1)*6, 2, '0', STR_PAD_LEFT).":00' ORDER BY heureMeteo ASC;";
				$rows = MySQL::wpdbSelect($strSql);
